...
CFuncPtrType_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	/* Fill in remaining fields in stgdict... */
	obj = PyDict_GetItemString(typedict, "_flags_"); /* Borrowed ref */
	if (!obj) {
		PyErr_SetString(PyExc_AttributeError,
				"class must define a '_flags_' attribute");
		return NULL;
	}
	flags = PyInt_AsLong(obj);
	if (flags == -1 && PyErr_Occurred()) {
		PyErr_SetString(PyExc_AttributeError,
				"'_flags_' attribute must be an integer");
		return NULL;
	}
	stgdict->flags = flags;
	...
	obj = PyDict_GetItemString(typedict, "_argtypes_"); /* Borrowed ref */
	if (!obj) {
		PyErr_SetString(PyExc_AttributeError,
				"class must define a '_argtypes_' attribute");
		return NULL;
	}
	obj = PySequence_Tuple(obj);
	if (!obj) {
		PyErr_SetString(PyExc_TypeError,
				"_argtypes_ must be a sequence");
		Py_DECREF(result);
		return NULL;
	}
	stgdict->argtypes = obj;

	size = PyTuple_GET_SIZE(obj);
	stgdict->converters = PyTuple_New(size);
	if (!stgdict->converters) {
		Py_DECREF(result);
		return NULL;
	}
	...
	stgdict->nArgBytes = 0;
	for (i = 0; i < size; ++i) {
		PyObject *ob = PyTuple_GET_ITEM(obj, i);
		StgDictObject *dict = PyType_stgdict(ob);
		PyObject *cnv = PyObject_GetAttrString(ob, "from_param");
		if (!dict || !cnv) {
			Py_DECREF(result);
			Py_XDECREF(cnv);
			PyErr_Format(PyExc_TypeError,
				     "_argtypes_ item %d not acceptable",
				     i+1);
			return NULL;
		}
		PyTuple_SET_ITEM(stgdict->converters, i, cnv);
...
static PyMethodDef CFuncPtrType_methods[] = {
	{ "from_param", CFuncPtrType_from_param, METH_O,
	  from_param_doc },
	{ "from_address", CDataType_from_address, METH_O,
	  "create an instance from an address"},
	{ NULL }
...
PyTypeObject CFuncPtrType_Type = {
	...
	0,					/* tp_as_buffer */
	Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE, /* tp_flags */
	"metatype for the FunctionPointer Objects",	/* tp_doc */
	0,					/* tp_traverse */
	0,					/* tp_clear */
	0,					/* tp_richcompare */
	0,					/* tp_weaklistoffset */
...
static PyObject *
CFuncPtr_FromCallable(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	CFuncPtrObject *self;
	StgDictObject *dict;
	PyObject *objects;
	...
	if (!dict || !dict->argtypes) {
		PyErr_SetString(PyExc_TypeError,
				"cannot create callback from this class");
		return NULL;
	}
	self->thunk = AllocFunctionCallback(callable, dict->nArgBytes,
					    dict->argtypes, dict->flags & FUNCFLAG_CDECL);
	*(void **)self->b_ptr = self->thunk;
	self->argtypes = dict->argtypes;
	Py_INCREF(self->argtypes);
	self->restype = dict->restype;
	Py_XINCREF(self->restype);
...
CFuncPtr_FromDll(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTuple(args, "sOi", &name, &dll, &flags))
		return NULL;
...
CFuncPtr_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	if (size == 1)
		return CFuncPtr_FromCallable(type, args, kwds);
	if (size == 3)
		return CFuncPtr_FromDll(type, args, kwds);
	PyErr_SetString(PyExc_TypeError,
			"CFuncPtr takes 1 or 3 arguments");
	return NULL;
}
...
static PyObject *
CFuncPtr_call(CFuncPtrObject *self, PyObject *args, PyObject *kw)
{
	if (self->argtypes) {
		int required, actual;
		required = PyTuple_GET_SIZE(self->argtypes);
		actual = PyTuple_GET_SIZE(args);
		if (actual != required) {
			PyErr_Format(PyExc_TypeError,
				     "function takes exactly %d argument%s"
				     " (%d given)",
				     required,
				     required == 1 ? "" : "s",
				     actual);
...
static PyGetSetDef CFuncPtr_getsets[] = {
	{ "_as_parameter_", (getter)CFuncPtr_as_parameter, NULL,
	  ...
	  NULL },
/* XXX getters and setters for restype and argtypes */
	{ NULL, NULL }
};
...
static PyTypeObject CFuncPtr_Type = {
	...
	0,					/* tp_descr_set */
	0,					/* tp_dictoffset */
	(initproc)CFuncPtr_init,		/* tp_init */
	0,					/* tp_alloc */
	CFuncPtr_new,				/* tp_new */
	0,					/* tp_free */
};
...
*/
...
#if 0
	{
		StgDictObject *dict = (StgDictObject *)PyObject_CallObject(
			(PyObject *)&StgDict_Type, NULL);
		dict->size = sizeof(void *);
		dict->length = 1;
		dict->align = getentry("P")->align;

		if (-1 == PyDict_Update((PyObject *)dict, CFuncPtr_Type.tp_dict))
			return;
